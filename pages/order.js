...
import { loadCart, total, clearCart, BRAND } from "../lib/cart";
...
export default function Order() {
  ...
  function placeOrder(e) {
    ...
    window.location.href = `https://wa.me/${BRAND.whatsapp}?text=${encodeURIComponent(text)}`;
    clearCart();
    setCart([]);
  }
  ...
  return (
    <>
      ...
      <main className="max-w-6xl mx-auto px-4 py-14">
        ...
        <form onSubmit={placeOrder} className="mt-6 bg-white rounded-2xl border p-6 shadow-card grid gap-3">
          ...
          {form.payment === 'UPI' && amount > 0 && (
            <a className="px-5 py-3 rounded-xl border border-brand text-brand font-semibold text-center" href={`upi://pay?pa=${BRAND.upiVPA}&pn=GoodMorningKitchen&am=${amount}&tn=GMK%20Order&cu=INR`}>
              Tap to Pay UPI ₹{amount}
            </a>
          )}
        </form>
      </main>
      ...
      <Footer />
      <MobileCartButton count={cart.reduce((s,i)=>s+i.qty,0)} />
    </>
  );
}
